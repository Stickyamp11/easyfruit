...
<form  class="create-product-form" @submit.prevent="">
  ...
  <div class="form-group row"  style="margin-top: 2%;">
    ...
    <button type="submit" class="btn-option-create-product btn btn-success"  v-on:click="handleSubmit">Actualizar</button>
     </div>
...
export default {
  ...
  methods: {
    ...
    async refreshData(){
      ...
      await storeService.getStoreProduct( this.storeInfo.id, this.productId).then(
        res => {
          ...
          this.setMethodsForProduct(this.product.methodsAllowed);
    ...
    setMethodsForProduct(arrayMethods){
      let methodsAllowed = []
      methodsAllowed = arrayMethods.split(';');
      methodsAllowed.forEach(method => {
        if(method == "kg")
        {
          this.acceptKg = true;
          this.pricePerKg = this.product.price_per_kg;
        }
        if(method == "pieces")
        {
          this.acceptUnits = true;
          this.pricePerUnit = this.product.price_per_unit;
        }
        if(method == "pack")
        {
          this.acceptPacks = true;
          this.pricePerPack = this.product.price_per_pack;
          this.packQuantity = this.product.packQuantity;
        }
